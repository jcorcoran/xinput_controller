...
void XINPUT::dpadUpdate(uint8_t dpadUP, uint8_t dpadDOWN, uint8_t dpadLEFT, uint8_t dpadRIGHT)
{
	...
	//DPAD Down
	if (dpadDOWN) {TXData[BUTTON_PACKET_1] |= DPAD_DOWN_MASK_ON;}
	//DPAD Left
	if (dpadLEFT) {TXData[BUTTON_PACKET_1] |= DPAD_LEFT_MASK_ON;}
	//DPAD Right
	if (dpadRIGHT) {TXData[BUTTON_PACKET_1] |= DPAD_RIGHT_MASK_ON;}
}
